...
const SAMPLE_QUOTES = [
  {
    ...
    message: "We need a bulk quotation for our monthly medicine procurement.",
    status: "new",
    date: "23 Sep 2026",
  },
  {
    ...
    message: "Please provide bulk pricing and availability.",
    status: "reviewing",
    date: "21 Sep 2026",
  },
];
...
export default function AdminDashboard() {
  ...
  const tabs = [
    { id: "orders", label: "Orders", icon: ShoppingCart, count: 24 },
    { id: "products", label: "Products", icon: Package, count: 4800 },
    { id: "customers", label: "Customers", icon: Users, count: 650 },
    { id: "quotes", label: "Quotes", icon: FileText, count: 18 },
  ];
...
function SummaryCard({ icon: Icon, label, value, trend, accent = "blue" }) {
...
function TableShell({ title, description, action, children }) {
  return (
...
const TH = "px-5 py-4 text-left text-[10px] font-black uppercase tracking-[0.14em] text-slate-500";
...
function ProductsTab() {
    ...
    setProducts((current) =>
      current.map((item) =>
        item.id === product.id
          ? {
              ...item,
              price: draft.price !== undefined ? Number(draft.price) : item.price,
              moq: draft.moq !== undefined ? Number(draft.moq) : item.moq,
              stock: draft.stock !== undefined ? Number(draft.stock) : item.stock,
            }
          : item
      )
    );
  ...
  const isDirty = (product) => Boolean(editing[product.id]);
      ...
      <table className="w-full min-w-[900px] text-left text-sm">
        ...
        <tbody className="divide-y divide-slate-100">
          {products.map((product) => {
            ...
            const stockLow = (draft.stock ?? product.stock) < 500;

            return (
              <tr
                key={product.id}
                className={`transition hover:bg-slate-50/70 ${
                  dirty ? "bg-[#0F4C81]/[0.03]" : ""
                }`}
              >
                <td className="px-5 py-4">
                ...
                <td className="px-5 py-4">
                  <div className="relative">
                    ...
                    <input
                      type="number"
                      value={draft.price !== undefined ? draft.price : product.price}
                      onChange={(event) =>
                        updateField(product.id, "price", event.target.value)
                      }
                ...
                <td className="px-5 py-4">
                  <input
                    type="number"
                    value={draft.moq !== undefined ? draft.moq : product.moq}
                    onChange={(event) =>
                      updateField(product.id, "moq", event.target.value)
                    }
                ...
                <td className="px-5 py-4">
                  <div className="flex items-center gap-2">
                    <input
                      type="number"
                      value={draft.stock !== undefined ? draft.stock : product.stock}
                      onChange={(event) =>
                        updateField(product.id, "stock", event.target.value)
                      }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-500">
          {quotes.length} active request{quotes.length !== 1 ? "s" : ""}
        </p>
      </div>
          ...
          <div className="flex flex-col justify-between gap-4 border-b border-slate-100 bg-slate-50/50 p-5 sm:flex-row sm:items-start">
            <div className="min-w-0">
              ...
              <p className="mt-1.5 text-xs text-slate-400">
                {quote.email} · {quote.phone} · {quote.date}
              </p>
            </div>
